...
// Existence checks decide whole scoring dimensions, and a miss is worth up to
// 1.5 points — enough to flip the #1 ranking and fire a false "we dropped"
// alert. Dynamically-generated manifests on scale-to-zero hosting can take ~7s
// on a cold start (colaberry.ai's own /llms-full.txt is a 354KB CMS-backed
// route), which used to blow the old 8s budget intermittently. So: a longer
// timeout plus one retry, and only a genuine two-attempt failure scores 0.
async function fetchHead(url, attempts = 2) {
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      clearTimeout(t);
      if (res.status === 200 || i === attempts - 1) return res.status;
    } catch {
      if (i === attempts - 1) return 0;
    }
    await new Promise((r) => setTimeout(r, 1500)); // let a cold instance warm up
  }
  return 0;
}
