...
- `src/lib/distribution/*` — Catalog distribution module (POC). `types.ts` contract, `source.ts` lean Strapi fetcher (per-kind isolation, `Promise.allSettled`), `templates.ts` pure per-platform renderer (X 280-char budget, Moltbook title+body, HF JSONL row), `orchestrator.ts` source → templates → dispatch pipeline with bounded per-platform concurrency and structured `DistributionRunResult`. Clients in `clients/` — `x.ts` OAuth 1.0a HMAC-SHA1 against `api.twitter.com/2/tweets`, `moltbook.ts` bearer-auth `/posts`, `huggingface.ts` stub (dry-run only; live commit deferred — see `docs/distribution/README.md` §HF Stub Rationale). Every client's `dispatch()` is contractually non-throwing. Idempotency key: `${platform}:${entry.id}:${entry.updatedAt}`.
- `src/pages/api/cron/catalog-distribution.ts` — Daily distribution cron. POST-only, bearer auth via `CATALOG_DISTRIBUTION_SECRET`, `Cache-Control: no-store`. **DRY_RUN by default** — live posting requires `CATALOG_DISTRIBUTION_LIVE=true` env OR `?live=true` query param.
- `src/pages/api/internal/distribution-preview.ts` — Admin DRY_RUN preview. GET|POST, admin-key auth, **always dry-run regardless of flags**. Query params: `windowHours`, repeatable `kind`, repeatable `platform`.
