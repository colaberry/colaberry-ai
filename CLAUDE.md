...
| `/demo/internal/[slug]` | **Staff-only** detail page (SSR, `noindex`, `private, no-store`). Gated in `getServerSideProps`: no session → redirect to `/login?redirect=…`; signed in but not staff → `notFound` (404, never a "forbidden" page, which would confirm the demo exists). |

**Internal (staff-only) demos.** Records live in `src/data/internalDemos.ts`, which is **server-only** — import it *only* from `pages/api/demos/internal.ts` and the `getServerSideProps` of `pages/demo/internal/[slug].tsx`. Never from a client component: `/demo` is static and public, so anything reachable from it ships to everyone. The hub fetches `/api/demos/internal` after mount and renders an "Internal demos" section only if the response is non-empty; that endpoint returns `{demos: []}` with a **200** (never 401/403) for everyone who isn't a signed-in staff member, so it can't be used to discover a hidden demo. Access = verified session email whose domain matches `INTERNAL_DEMO_EMAIL_DOMAINS` (default `colaberry.com`), via `isInternalDemoViewer()` in `src/lib/auth/internalAccess.ts` — exact domain after the LAST `@`, so `notcolaberry.com` and `colaberry.com.evil.com` don't match. Internal demos are deliberately absent from the hub's `ItemList` JSON-LD, the sitemap and `llms.txt`. Note the gate controls *listing on colaberry.ai only* — it says nothing about whether the demo's own host requires auth.
...
- `src/lib/ai-dev-wire/*` + `src/pages/ai-dev-wire.tsx` + `src/pages/api/ai-dev-wire.ts` — **AI Dev Wire** daily AI-dev trends board (Ram's directive). Pluggable **N-source** engine: `sources.ts` (6 `SourceAdapter`s — HF trending models, HF Daily Papers, Hacker News, GitHub Trending, vendor blogs [8 RSS], arXiv — each failure-isolated via `Promise.allSettled`), `relevance.ts` (word-boundary AI/dev gate + consumer-noise blocklist + model-variant dedup), `recommend.ts` (cross-source **corroboration** ranking — same entity across independent sources; one pick per story/kind), `index.ts` (`getWire(force)` — 30-min in-memory cache, 60s min-refresh, stale-if-error). API `GET /api/ai-dev-wire` (`?refresh=1` forces a re-run). Page is on-brand (SectionHeader hero, Refresh control, "6 sources live" health line, 3 "Start here" picks, filterable feeds, EnterpriseCtaBand). Wired into nav under **Resources → AI Dev Wire** (`/ai-dev-wire`), visible by default. Adding a source = one adapter, no page/API change. Standalone origin + daily cloud refresh: `saitejesh-cyber/ai-dev-wire` / `~/ai-trends`.
